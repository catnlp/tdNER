...
'''
from util.metric import get_ner_fmeasure
...
def evaluate(data, model, name, ignore=False):
    ...
    return speed, acc, p, r, f, pred_results, gold_results, f_results
...
def train(data, name, save_dset, save_model_dir, seg=True, ignore=False, cove_flag=False):
    ...
    for idx in range(epoch):
        ...
        for batch_id in range(total_batch):
            ...
            instance_count += 1
            loss, tag_seq = model.neg_log_likelihood_loss(batch_word, batch_wordlen, batch_label, mask)
        ...
        speed, acc, p, r, f_train, _, _, dict_train = evaluate(data, model, 'train', ignore=ignore)
        # speed, acc, p, r, f_dev, dict_dev = evaluate(data, model, 'dev', ignore=ignore)
        speed, acc, p, r, f_test, _, _, dict_test = evaluate(data, model, 'test', ignore=ignore)
        ...
        vis.line(np.array(all_F), X=np.array([i for i in range(len(all_F))]),
                 win=Fwin, opts={'title': Fwin, 'legend': ['train', 'test']})
        ...
        if dict_train:
            for key, value in dict_train.items():
                if key not in label_F:
                    dict_F[key] = [[0., 0.]]
                    label_F.append(key)
                dict_F[key].append([dict_train[key]*100.0, dict_test[key]*100.0])
...
def load_model_decode(model_dir, data, name, gpu, seg=True):
    ...
    start_time = time.time()
    speed, acc, p, r, f, pred_results, gold_results, _ = evaluate(data, model, name)
    end_time = time.time()
    time_cost = end_time - start_time
    if seg:
        print('%s: time: %.2f, speed:%.2ft/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f'
...
def test_optimizer(data):
    ...
    for idx in range(epoch):
        ...
        speed, acc, p, r, f_train_SGD, _, _, _ = evaluate(data, model_SGD, 'train')
        speed, acc, p, r, f_train_Adam, _, _, _ = evaluate(data, model_Adam, 'train')
        speed, acc, p, r, f_train_RMSprop, _, _, _ = evaluate(data, model_RMSprop, 'train')
        speed, acc, p, r, f_train_Adadelta, _, _, _ = evaluate(data, model_Adadelta, 'train')
        speed, acc, p, r, f_train_Adagrad, _, _, _ = evaluate(data, model_Adagrad, 'train')
        ...
        speed, acc, p, r, f_test_SGD, _, _, _ = evaluate(data, model_SGD, 'test')
        speed, acc, p, r, f_test_Adam, _, _, _ = evaluate(data, model_Adam, 'test')
        speed, acc, p, r, f_test_RMSprop, _, _, _ = evaluate(data, model_RMSprop, 'test')
        speed, acc, p, r, f_test_Adadelta, _, _, _ = evaluate(data, model_Adadelta, 'test')
        speed, acc, p, r, f_test_Adagrad, _, _, _ = evaluate(data, model_Adagrad, 'test')
