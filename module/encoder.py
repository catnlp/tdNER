...
@Time: 2018/5/2 15:01
'''
from module.component.RNNs import RNN, LSTM
from module.component.MetaRNNs import MetaRNN, MetaLSTM
# from module.char import Char
...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        if self.mode == 'BaseRNN':
            ...
            self.encoder = RNN(self.embedding_dim, self.hidden_dim, num_layers=self.layers, gpu=self.gpu)
        elif self.mode == 'MetaRNN':
            self.encoder = MetaRNN(self.embedding_dim, self.hidden_dim, self.hyper_hidden_dim, self.hyper_embedding_dim, num_layers=self.layers, gpu=self.gpu)
        elif self.mode == 'BaseLSTM':
            self.encoder = nn.LSTM(self.embedding_dim, self.hidden_dim//2, num_layers=self.layers, batch_first=True, bidirectional=True)
        elif self.mode == 'LSTM':
            self.encoder = LSTM(self.embedding_dim, self.hidden_dim//2, num_layers=self.layers, gpu=self.gpu, bidirectional=self.bidirectional)
        elif self.mode == 'MetaLSTM':
            self.encoder = MetaLSTM(self.embedding_dim, self.hidden_dim//2, self.hyper_hidden_dim, self.hyper_embedding_dim, num_layers=self.layers, gpu=self.gpu, bidirectional=self.bidirectional)
